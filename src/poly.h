...
/**
 * To jest makro oznaczające błędne Mono, którego nie da się stworzyć
 * w normalnych warunkach. Istnieje po to, aby funkcja ParseMono mogła
 * poinformować o wystąpieniu błędu w trakcie parsowania napisu.
 */
#define ERR_MONO (Mono) { .p = PolyZero(), .exp = -1 }

/**
 * To jest makro oznaczające błędne Poly, którego nie da się stworzyć
 * w normalnych warunkach. Istnieje po to, aby funkcja ParsePoly mogła
 * poinformować o wystąpieniu błędu w trakcie parsowania napisu.
 */
#define ERR_POLY (Poly) { .arr = (Mono *) str, .size = 0 }

/**
 * To jest makro sprawdzające wskaźnik, aby dowiedzieć się,
 * czy udała się alokacja pamięci.
 */
#define CHECK_PTR(p)    \
  do {                  \
    if ((p) == NULL) {  \
...
/**
 * Sprawdza, czy zadany wielomian jest wielomianem błędnym,
 * tj. informującym o błędzie w trakcie parsowania napisu.
 * @param[in] p : wielomian
 * @return czy parsowanie się udało
 */
static inline bool PolyIsErr(const Poly *p) {
  return (p->arr != NULL && p->size == 0);
}

/**
 * Sprawdza, czy zadany jednomian jest jednomianem błędnym,
 * tj. informującym o błędzie w trakcie parsowania napisu.
 * @param[in] m : jednomian
 * @return czy parsowanie się udało
 */
static inline bool MonoIsErr(const Mono *m) {
  return (MonoGetExp(m) == -1);
}
...
static inline bool PolyIsZero(const Poly *p) {
  if (PolyIsCoeff(p))
    return (p->coeff == 0);
  return (p->size == 1 && PolyIsZero(&p->arr[0].p));
}
...
/**
 * Porównuje dwa jednomiany na podstawie ich wykładników.
 * @param[in] a : jednomian
 * @param[in] b : jednomian
 * @return a.exp @f$>@f$ b.exp
 */
int CompareMonos(const void *a, const void *b);

/**
 * Sortuje tablicę jednomianów na podstawie ich wykładników.
 * @param[in] count : liczba jednomianów
 * @param[in] monos : tablica jednomianów
 */
static inline void SortMonos(size_t count, Mono monos[]) {
...
/**
 * Wykonuje operację składania, która dla zadanego wielomianu @p p
 * i @p k wielomianów @f$q[k - 1], q[k - 2], ..., q[0]@f$ zwraca
 * wielomian @f$p(q_0, q_1, q_2, ...)@f$ powstający poprzez podstawienie
 * w wielomianie @p p pod zmienną @f$x_i@f$ wielomianu @f$q_i@f$ dla
 * @f$i = 0, 1, 2, ..., \min(k, l) - 1@f$. Jeśli @f$k<l@f$, to pod zmienne
 * @f$x_k, ..., x_{l−1}@f$ podstawiamy zera.
